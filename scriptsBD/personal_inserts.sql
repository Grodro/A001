...
order by sal_emp ;
#09 ALIAS
SELECT nombre AS Nombre_Empleado, cargo_emp AS Puesto from empleados;
#10
select *from empleados
where id_depto=2000
...
#19
SELECT * FROM departamentos
WHERE NOT nombre_depto LIKE 'VENTAS' AND NOT nombre_depto LIKE 'Investigacion' AND NOT nombre_depto LIKE 'Mantenimiento';
#20
SELECT nombre, MAX(sal_emp) FROM empleados;
#21
SELECT nombre, id_emp FROM empleados
ORDER BY nombre desc
LIMIT 1;
#22
...
WHERE id_depto=1000 OR id_depto=2200;
SELECT AVG(sal_emp) FROM empleados
WHERE id_depto=1500;
SELECT AVG(sal_emp) FROM empleados
WHERE id_depto=3500;
...
SELECT COUNT(id_depto),id_depto FROM empleados GROUP BY id_depto
HAVING COUNT(id_depto) >3;
#SELECT COUNT(id_emp), id_depto FROM empleados GROUP BY id_depto# HAVING COUNT(id_emp) >= 3;

#25 No hay forma de relacionar el Nombre del jefe con el numero de jefe
SELECT COUNT(id_depto)AS CANTIDAD_EMPLEADOS,id_depto,cod_jefe FROM empleados GROUP BY cod_jefe
HAVING COUNT(id_depto) >=2;

SELECT cod_jefe, nombre, count(*) as 'Num empleados', cargo_emp
FROM empleados 
...
SELECT nombre,sal_emp,id_depto FROM empleados GROUP BY id_depto
HAVING sal_emp>AVG(sal_emp);
# ORDER BY id_depto;
SELECT AVG(sal_emp) from empleados;
